...
class SSLMetaArch(LightningModule):
    ...
    def batch_forward_backbone(
        ...
        outputs = backbone(token_embed, embed, token_pos, pos)
        outputs["path_tokens"] = rearrange(
            outputs["path_tokens"], "(b n) ... -> b n ...", b=b
        )
        outputs["cls_tokens"] = rearrange(
            outputs["cls_tokens"], "(b n) ... -> b n ...", b=b
        )
        return outputs
    ...
    def forward(self, batch: dict[str, Any]) -> dict[str, Tensor]:
        ...
        global_outputs = self.batch_forward_backbone(
            self.student.backbone, batch["global_crops"], batch["global_crop_tokens"]
        )
    ...
    def training_step(self, batch: dict[str, Any]) -> Tensor:
        ...
        ibot_loss = self.ibot_patch_loss(
            outputs["global_patch_logits"], targets["global_patch_logits"]
        )
        dino_loss = self.dino_loss(
            outputs["global_cls_logits"], targets["global_cls_logits"]
        )
